@extends('layouts.admin', ['pageTitle' => 'Edit Scholarship Rules'])

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-on-surface">Edit Scholarship Rules</h1>
        ...
        <div class="border-t border-outline-variant pt-6">
            ...
            <div>
                ...
                <select id="required_nationality" name="required_nationality" class="input @error('required_nationality') input-error @enderror">
                    ...
                    <option value="Malaysian" {{ old('required_nationality', $rule->required_nationality) === 'Malaysian' ? 'selected' : '' }}>Malaysian</option>
                    <option value="Singaporean" {{ old('required_nationality', $rule->required_nationality) === 'Singaporean' ? 'selected' : '' }}>Singaporean</option>
                    <option value="Indonesian" {{ old('required_nationality', $rule->required_nationality) === 'Indonesian' ? 'selected' : '' }}>Indonesian</option>
                    <option value="Bruneian" {{ old('required_nationality', $rule->required_nationality) === 'Bruneian' ? 'selected' : '' }}>Bruneian</option>
                    <option value="Other" {{ old('required_nationality', $rule->required_nationality) === 'Other' ? 'selected' : '' }}>Other</option>
                </select>
            ...
            <div>
                ...
                <select id="required_study_level" name="required_study_level" class="input @error('required_study_level') input-error @enderror">
                    ...
                    @foreach(['SPM', 'STPM', 'Foundation', 'Matriculation', 'Diploma', 'Undergraduate'] as $level)
                        <option value="{{ $level }}" {{ old('required_study_level', $rule->required_study_level) === $level ? 'selected' : '' }}>{{ $level }}</option>
                    @endforeach
                </select>
            ...
            <div class="grid sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" id="min_spm_as" name="min_spm_as" min="0" max="12" class="input @error('min_spm_as') input-error @enderror"
                        value="{{ old('min_spm_as', $rule->min_spm_as) }}" placeholder="0-12">
                    @error('min_spm_as')
                        <p class="mt-1 text-sm text-error">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="number" id="min_spm_credits" name="min_spm_credits" min="0" max="12" class="input @error('min_spm_credits') input-error @enderror"
                        value="{{ old('min_spm_credits', $rule->min_spm_credits) }}" placeholder="0-12">
                ...
                <div>
                    ...
                    <input type="number" id="min_cgpa" name="min_cgpa" step="0.01" min="0" max="4" class="input @error('min_cgpa') input-error @enderror"
                        value="{{ old('min_cgpa', $rule->min_cgpa) }}" placeholder="0.00-4.00">
                    @error('min_cgpa')
                        <p class="mt-1 text-sm text-error">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="border-t border-outline-variant pt-6">
            ...
            <div>
                ...
                <select id="required_income_category" name="required_income_category" class="input @error('required_income_category') input-error @enderror">
                    ...
                    <option value="B40" {{ old('required_income_category', $rule->required_income_category) === 'B40' ? 'selected' : '' }}>B40 (Bottom 40%)</option>
                    <option value="M40" {{ old('required_income_category', $rule->required_income_category) === 'M40' ? 'selected' : '' }}>M40 (Middle 40%)</option>
                    <option value="T20" {{ old('required_income_category', $rule->required_income_category) === 'T20' ? 'selected' : '' }}>T20 (Top 20%)</option>
                </select>
            ...
            <div>
                ...
                <input type="number" id="max_household_income" name="max_household_income" step="0.01" min="0" class="input @error('max_household_income') input-error @enderror"
                    value="{{ old('max_household_income', $rule->max_household_income) }}" placeholder="e.g., 3169.00">
        ...
        <div class="border-t border-outline-variant pt-6">
            ...
            <div>
                ...
                <input type="text" id="required_field_of_study" name="required_field_of_study" class="input @error('required_field_of_study') input-error @enderror"
                    value="{{ old('required_field_of_study', $rule->required_field_of_study) }}" placeholder="e.g., Engineering, Medicine, Business, Computer Science">
            ...
            <div>
                ...
                <select id="required_institution_type" name="required_institution_type" class="input @error('required_institution_type') input-error @enderror">
                    ...
                    @foreach(['Public University', 'Private University', 'Polytechnic', 'Community College', 'Foundation College', 'Other'] as $type)
                        <option value="{{ $type }}" {{ old('required_institution_type', $rule->required_institution_type) === $type ? 'selected' : '' }}>{{ $type }}</option>
                    @endforeach
                </select>
            ...
            <div class="grid sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="income_rule_type" name="income_rule_type" class="input @error('income_rule_type') input-error @enderror" required>
                        <option value="none" {{ old('income_rule_type', $rule->income_rule_type) === 'none' ? 'selected' : '' }}>None</option>
                        <option value="soft" {{ old('income_rule_type', $rule->income_rule_type) === 'soft' ? 'selected' : '' }}>Soft (scoring)</option>
                        <option value="hard" {{ old('income_rule_type', $rule->income_rule_type) === 'hard' ? 'selected' : '' }}>Hard (filter)</option>
                    </select>
                ...
                <div>
                    ...
                    <select id="study_level_rule_type" name="study_level_rule_type" class="input @error('study_level_rule_type') input-error @enderror" required>
                        <option value="none" {{ old('study_level_rule_type', $rule->study_level_rule_type) === 'none' ? 'selected' : '' }}>None</option>
                        <option value="soft" {{ old('study_level_rule_type', $rule->study_level_rule_type) === 'soft' ? 'selected' : '' }}>Soft (scoring)</option>
                        <option value="hard" {{ old('study_level_rule_type', $rule->study_level_rule_type) === 'hard' ? 'selected' : '' }}>Hard (filter)</option>
                    </select>
                ...
                <div>
                    ...
                    <select id="field_rule_type" name="field_rule_type" class="input @error('field_rule_type') input-error @enderror" required>
                        <option value="none" {{ old('field_rule_type', $rule->field_rule_type) === 'none' ? 'selected' : '' }}>None</option>
                        <option value="soft" {{ old('field_rule_type', $rule->field_rule_type) === 'soft' ? 'selected' : '' }}>Soft (scoring)</option>
                        <option value="hard" {{ old('field_rule_type', $rule->field_rule_type) === 'hard' ? 'selected' : '' }}>Hard (filter)</option>
                    </select>
                ...
                <div>
                    ...
                    <select id="institution_rule_type" name="institution_rule_type" class="input @error('institution_rule_type') input-error @enderror" required>
                        <option value="none" {{ old('institution_rule_type', $rule->institution_rule_type) === 'none' ? 'selected' : '' }}>None</option>
                        <option value="soft" {{ old('institution_rule_type', $rule->institution_rule_type) === 'soft' ? 'selected' : '' }}>Soft (scoring)</option>
                        <option value="hard" {{ old('institution_rule_type', $rule->institution_rule_type) === 'hard' ? 'selected' : '' }}>Hard (filter)</option>
                    </select>
